...
function LocationCard({ location, chars, factions, stories, onEdit, onDelete, isSelected, onSelect }) {
  ...
  return (
    <CardRow isSelected={isSelected} onClick={()=>onSelect(location)}>
      <div style={{ display:"flex", alignItems:"flex-start", gap:10 }}>
        ...
        <div style={{ display:"flex", gap:5, flexShrink:0, alignItems:"center" }} onClick={e=>e.stopPropagation()}>
          ...
          {confirmDelete
            ? <>
                <button onClick={()=>{ onDelete(location.id); setConfirmDelete(false); }} style={{...iconBtn, color:"#c06060", fontWeight:700}}>✓</button>
                <button onClick={()=>setConfirmDelete(false)} style={iconBtn}>✕</button>
              </>
            : <button onClick={()=>setConfirmDelete(true)} style={{...iconBtn,color:"#c06060"}}>🗑️</button>
          }
        </div>
      </div>
    </CardRow>
        ...
        <div style={{ flex:1, minWidth:0, overflowY:"auto", paddingRight:12 }}>
          ...
          {locations.length===0
            ...
            : (()=>{
                ...
                if(!grouped.length) return <div style={{ textAlign:"center", padding:"40px 0", color:"#5a4a7a", fontSize:13 }}>No locations match. Try clearing the search or filters.</div>;
